...
export default function ProductDetailModal({ isOpen, onClose, slug }: ProductDetailModalProps) {
    ...
    publicProductsApi.getBySlug(slug)
      .then(data => {
        ...
        if (data.primary_image_url) {
          setActiveImage(`${import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000'}${data.primary_image_url}`);
      ...
      .catch(err => {
        console.error(err);
        onClose(); // Close modal if not found
      })
          ...
          {product.category !== 'cards' && (
            <div className="mb-8 space-y-6">
              <div className="flex flex-col">
                <span className="text-sm text-primary-500">وقت التجهيز</span>
                <span className="font-bold text-primary-900">{product.preparation_time_minutes} دقيقة</span>
              </div>
