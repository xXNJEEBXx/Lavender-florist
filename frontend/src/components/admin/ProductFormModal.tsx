  ...
  useEffect(() => {
    if (product) {
      ...
      if (product.images && product.images.length > 0) {
        setImagePreviews(product.images.map(img => `${import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000'}${img.image_url}`));
      } else if (product.primary_image_url) {
        setImagePreviews([`${import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000'}${product.primary_image_url}`]);
      } else {
        setImagePreviews([]);
      }
                  ...
                  return (
                    <div key={index} className="flex gap-4 items-center bg-white p-3 rounded-xl border border-primary-100">
                      {selectedComp?.image_url ? (
                        <img src={`${import.meta.env.VITE_API_BASE_URL || 'http://127.0.0.1:8000'}${selectedComp.image_url}`} alt={selectedComp.name} className="w-10 h-10 rounded-lg object-cover border border-primary-100 flex-shrink-0" />
                      ) : (
